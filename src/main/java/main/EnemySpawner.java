...
public class EnemySpawner {
        ...
        try {
            ...
            for (int row = 0; row < rows; row++) {
                ...
                for (int col = 0; col < numbers.length; col++) {
                    ...
                    if (value == 1) {
                        enemyQueues.get(row).add(new Enemy(new Coordinate(0,0,gp),1,1,gp));
    ...
    private void spawnEnemy() {
        ...
        // removes the element in the front of that rounds queue and adds it to the enemies ArrayList
        
        if (enemyQueues != null && cooldownTimer <= 0){
            gp.enemies.add(enemyQueues.get(round-1).peek());
            enemyQueues.get(round-1).poll();
            cooldownTimer = SPEED;
        }
